...
const AdminUser = () => {
  ...
  const submitCoin = async () => {
    ...
    try {
      const res = await axios.put(`/update-coin/${coin.id}`, {
        balanceCoin: Number(coin.coin) + Number(beforeCoin),
      });
      if (res.status === 200) {
        await fetching();
        ...
        if (userHere) {
          await addInvoice(
            Number(coin.coin) + Number(beforeCoin),
            coin.id,
            userHere.data.name ?? "Hi"
          );
          toast.success("Nạp tiền cho khách thành công");
          setCoin({
            id: "",
            coin: "",
          });
  ...
  return (
    <div className="">
      ...
      <h2 className="font-bold text-black-300 px-6 pb-1 text-2xl">
        Quản lý người dùng
      </h2>
      ...
      <Space className="mt-3 ml-5">
        ...
        <Input
          value={coin.coin}
          onChange={(e) => changeValueCoin("coin", e.target.value)}
        />
